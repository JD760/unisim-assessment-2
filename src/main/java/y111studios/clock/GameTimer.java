...
/**
 * An interface for a game timer. The timer can be paused and resumed, and has a
 * maximum duration
 * for the game. There are methods to check if the timer has reached the maximum
 * duration, as well
...
public interface GameTimer {
   ...
   */
  int INITIAL_SECONDS = 60;
  /**
   * The maximum duration of the timer.
   */
  Duration INITIAL_DURATION = Duration.ofSeconds(INITIAL_SECONDS);
  ...
  /**
   * Pauses the timer. The timer will not increment while paused and can be
   * resumed later.
   */
  void pause();
