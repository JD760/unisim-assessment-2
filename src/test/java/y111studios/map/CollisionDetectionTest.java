 ...
 * Tests FR_BUILDING_PLACEMENT, FR_PLACEMENT_VALIDITY, FR_OBSTACLES
 */
public class CollisionDetectionTest {
  ...
  private static Stream<Arguments> provideCanPlaceBuildingCases() {
    ...
    CollisionDetection rectY = new CollisionDetection(10, 20);
    Stream<Arguments> rectCasesY = Stream.of(
        Arguments.of(rectY, 1, 1, 3, 3, true), // Typical case
        Arguments.of(rectY, 4, 15, 4, 3, true), // Case: correct check of y vs. height
        Arguments.of(rectY, 15, 6, 2, 3, false) // Case: correct check of x vs. width
    );
    CollisionDetection rectX = new CollisionDetection(20, 10);
    Stream<Arguments> rectCasesX = Stream.of(
        Arguments.of(rectX, 1, 1, 3, 3, true), // Typical case
        Arguments.of(rectX, 15, 4, 3, 4, true), // Case: correct check of y vs. height
        Arguments.of(rectX, 6, 15, 3, 2, false) // Case: correct check of x vs. width
    );
    Stream<Arguments> rectCases = Stream.concat(rectCasesX, rectCasesY);
   ...
   * and then checking if buildings are blocked from being placed on top of each
   * other.
   */
  ...
  /**
   * Tests collision detection when attempting to remove buildings, firstly
   * checking
